...
# inserire la funzione di voce a testo
...
# Consegna #4. Implementare l'autorizzazione degli utenti
...
def login():
    ...
    if request.method == 'POST':
        ...
        # Ottenere tutti gli utenti dal database
        users_db = User.query.all()

        # Verificare i dati di accesso dell'utente rispetto alle voci del database
        for user in users_db:
            if form_login == user.login and form_password == user.password:
                return redirect('/index')
        else: 
        # Se nessuna corrispondenza viene trovata, restituisci un errore
